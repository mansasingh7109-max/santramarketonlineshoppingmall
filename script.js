...
// Date: 13-June-2026 - YOU CHOOSE FEATURE + FIREBASE SAFE + CLEAN
    ...
    window.db.collection("products").onSnapshot(
        ...
        error => {
            ...
            document.getElementById("productGrid").innerHTML =
                '<div class="loading">⚠️ Products load nahi hue. Firebase rules check karo.</div>';
            window.allProductsCache = SANTRA_DB.products || [];
            loadProducts();
        }
    );

    window.db.collection("categories").onSnapshot(
        snapshot => {
...
window.loadCategories = function () {
    ...
    if (!window.allCategoriesCache || window.allCategoriesCache.length === 0) {
        catContainer.innerHTML =
            '<p style="padding:20px;color:#999">No categories</p>';
        return;
    }
    ...
    try {
        ...
        if (item) {
            ...
            CART[productId] = {
                ...
                addedAt: Date.now(),
                addedAtIST: new Date().toLocaleString("en-IN", {
...
function updateCartCount() {
    ...
    if (navCartCountEl) {
        ...
        navCartCountEl.style.display = count > 0 ? "block" : "none";
    }
}
...
// 7. YOU CHOOSE FEATURE - NEW ADDITION
function moveToYouChoose(productId) {
    let CART = JSON.parse(localStorage.getItem(CART_KEY)) || {};
    let productToMove = CART[productId];
    ...
    const userId = window.auth?.currentUser?.uid || "guest";
    window.db
        .collection("users")
        .doc(userId)
        .collection("cart")
    ...
    // YOU CHOOSE me add karo
    let youChooseList = JSON.parse(localStorage.getItem(YOU_CHOOSE_KEY)) || [];
    if (!youChooseList.find(item => item.id === productId)) {
        youChooseList.push({
            id: productId,
            name: productToMove.name,
            price: productToMove.price,
            image: productToMove.image || productToMove.imageUrl,
            productCode:
                productToMove.product_code || productToMove.productCode,
            addedAt: new Date().toLocaleString("en-IN", {
                timeZone: "Asia/Kolkata"
            })
        });
        localStorage.setItem(YOU_CHOOSE_KEY, JSON.stringify(youChooseList));
    }

    updateCartCount();
    alert("✨ Moved to YOU CHOOSE!");

    // Cart page refresh karo
    if (typeof renderCart === "function") renderCart();
}

function renderYouChoose() {
    const youChooseContainer = document.getElementById("youChooseList");
    if (!youChooseContainer) return;

    let youChooseList = JSON.parse(localStorage.getItem(YOU_CHOOSE_KEY)) || [];

    if (youChooseList.length === 0) {
        youChooseContainer.innerHTML = `
            <div class="you-choose-card">
                <h3>✨ YOU CHOOSE – Your Personal Picks</h3>
                <p>Yahan aap apne favorite products save kar sakte hain.</p>
                <p style="margin-top:15px;color:var(--accent);font-weight:bold">Abhi koi item save nahi hai</p>
                <button onclick="window.location.href='index.html'" style="margin-top:20px;padding:12px 30px;background:var(--accent);color:white;border:none;border-radius:8px;font-weight:bold;cursor:pointer">Browse Products</button>
            </div>
        `;
        return;
    }

    youChooseContainer.innerHTML = youChooseList
        .map(
            item => `
        <div class="cart-card">
            <img src="${item.image}" onerror="this.src='https://via.placeholder.com/80x80/4CAF50/ffffff?text=NO+IMAGE'">
            <div class="item-info">
                <h3>${item.name}</h3>
                <div class="item-price">₹${item.price}</div>
                <div class="item-time">🕐 Saved: ${item.addedAt}</div>
            </div>
            <div class="item-actions">
                <button onclick="moveToCartFromYouChoose('${item.id}')" class="btn-movetocart">🛒 Move to Cart</button>
                <button class="btn-remove" onclick="removeFromYouChoose('${item.id}')">Delete</button>
            </div>
        </div>
    `
        )
...
function moveToCartFromYouChoose(productId) {
    let youChooseList = JSON.parse(localStorage.getItem(YOU_CHOOSE_KEY)) || [];
    let productToMove = youChooseList.find(item => item.id === productId);
    if (!productToMove) return;

    // YOU CHOOSE se remove karo
    youChooseList = youChooseList.filter(item => item.id !== productId);
    localStorage.setItem(YOU_CHOOSE_KEY, JSON.stringify(youChooseList));

    // Cart me add karo
    addToCart(productId);
    renderYouChoose();
    alert("🛒 Moved back to Cart!");
}

function removeFromYouChoose(productId) {
    if (confirm("Remove from YOU CHOOSE?")) {
        let youChooseList =
            JSON.parse(localStorage.getItem(YOU_CHOOSE_KEY)) || [];
        youChooseList = youChooseList.filter(item => item.id !== productId);
        localStorage.setItem(YOU_CHOOSE_KEY, JSON.stringify(youChooseList));
        renderYouChoose();
    }
}
// ===== YOU CHOOSE FEATURE END =====
...
function loadProducts(category = null) {
    let products =
        window.allProductsCache && window.allProductsCache.length > 0
            ? window.allProductsCache
            : SANTRA_DB.products;
...
// 11. renderProducts - WISHLIST UPDATED
...
function openProductModal(productId) {
    ...
    document.getElementById("modalMrp").innerText = product.mrp
        ? `₹${product.mrp}`
        : "";
    ...
    const modalWishBtn = document.querySelector(
        '#productModal.btn[onclick="addToWishlistFromModal()"]'
    );
    if (modalWishBtn) {
...
function changeModalSlide(direction) {
    ...
    if (dots.length > 0)
        dots[window.currentModalSlide].classList.remove("active");
    ...
    if (window.currentModalSlide < 0)
        window.currentModalSlide = slides.length - 1;
...
function addToCartFromModal() {
    if (window.currentModalProduct) addToCart(window.currentModalProduct.id);
}
function buyNowFromModal() {
    if (window.currentModalProduct) showOrderFormInModal();
}
...
function loginCustomerWithoutOTP() {
    ...
    localStorage.setItem(
        "santra_customer",
        JSON.stringify({ name: name, mobile: mobile })
    );
...
async function submitOrderFromPopup() {
    try {
        ...
        const address = document
            .getElementById("orderCustAddress")
            .value.trim();
        ...
        const qty =
            parseInt(document.getElementById("modalQty").innerText) || 1;
    ...
    `;
        ...
        const adminWhatsapp =
            SANTRA_DB.settings?.helpdeskWhatsapp || "8769171078";
        ...
        window.open(
            `https://wa.me/91${adminWhatsapp}?text=${whatsappMsg}`,
            "_blank"
        );
    } catch (err) {
